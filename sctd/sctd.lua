...
function SCTD:ShowSCTDMenu()
	...
	if (loaded) then
		...
		if (SCTDOptions) then
			...
			if (not SCTD:IsHooked(SCT2, "ShowExample")) then
				SCTD:RawHook(SCT2, "ShowExample")
			end
			--Hook SCT ShowTest
			if (not SCTD:IsHooked(SCT2, "ShowTest")) then
				SCTD:RawHook(SCT2, "ShowTest")
			end
        ...
        menuloaded = true
			...
			--open sct window
			SCTD.hooks[SCT2].ShowMenu()
			--open to SCTD menu
			InterfaceOptionsFrame_OpenToCategory("SCTD "..SCT2.LOCALS.OPTION_MISC104.name)
		...
		PlaySound(PlaySoundKitID and "TellMessage" or 3081)
		SCTD:Print(SCT2.LOCALS.Load_Error.." "..message)
	end
end
...
function SCTD:ParseCombat(larg1, timestamp, event, hideCaster, sourceGUID, sourceName, sourceFlags, sourceFlags2, destGUID, destName, destFlags, destFlags2, ...)
  ...
  elseif etype == "INTERRUPT" then
    ...
    self:DisplayText("SCTD_SHOWINTERRUPT", SCT2.LOCALS.Interrupted, nil, nil, nil, destName, extraSpellName, texture, destFlags, destGUID)
	end
end
	...
	if (db[option]) then
		...
		if (crit) then
			if (db["SCTD_SHOWCOLORCRIT"]) then
				rbgcolor = db[SCT2.COLORS_TABLE]["SCTD_SHOWCOLORCRIT"]
			end
